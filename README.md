...
- 黒画面検出
- 終了前の待機時間
- 黒画面以外の追加センサー / より細かい条件式
- インストーラー / 自動アップデート

## Dock UI

**条件の組み合わせ:** ドックの「条件の組み合わせ」で **または (OR)** / **かつ (AND)** を選べます。
- **または:** ON の静止・無音・メディア終了のどれか 1 つで録画終了
- **かつ:** ON の静止・無音・メディア終了がすべて満たされたら録画終了
- **タイマー（最大録画時間）** はモードに関係なく、単独で録画終了します

録画保存先フォルダは、組み合わせ行右のフォルダアイコンから開けます。
...
| 条件の組み合わせ | または / かつ（静止・無音・メディア終了）。タイマーは常に単独 |
